...
class AsyncTelegram:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self._loop_tasks = []
        self._loop_tasks_no_cancel = set()
        ...
        self._tdjson_executor = ThreadPoolExecutor(max_workers=1)
    ...
    def run(self):
        ...
        for i in range(self.settings.tdjson_workers):
            self.create_task(self._tdjson_worker(i), no_cancel=True)
        for i in range(self.settings.handlers_workers):
            self.create_task(self._handlers_worker(i), no_cancel=True)
        self.run_forever()
    ...
    def cancel_tasks(self):
        ...
        [
            ...
            if id(task) not in self._loop_tasks_no_cancel
        ]
    ...
    def _prepare_update(self, update: dict):
        try:
            ...
            self.logger.error('failed to parse update: %s', update)
            raise
...
class Authorization:
    ...
    def run(self, timeout):
        ...
        old_tdjson_workers = self.client.settings.tdjson_workers
        self.client.settings.tdjson_workers = 1
        old_tdjson_worker_tasks = self.client.settings.tdjson_worker_tasks
        self.client.settings.tdjson_worker_tasks = 1
        old_handlers_workers = self.client.settings.handlers_workers
        self.client.settings.handlers_workers = 1
        old_handlers_worker_tasks = self.client.settings.handlers_worker_tasks
        self.client.settings.handlers_worker_tasks = 1
        try:
            ...
            self.client.settings.tdjson_workers = old_tdjson_workers
            self.client.settings.tdjson_worker_tasks = old_tdjson_worker_tasks
            self.client.settings.handlers_workers = old_handlers_workers
            self.client.settings.handlers_worker_tasks = old_handlers_worker_tasks
