...
from .files import Animation, Audio, Contact
...
message_content = (
    'messageAnimation, messageAudio, messageBasicGroupChatCreate, '
    'messageCall, messageChatAddMembers, messageChatChangePhoto, '
    ...
    'messageChatUpgradeTo, messageContact, messageContactRegistered, '
    'messageCustomServiceAction, messageDice, messageDocument, '
    'messageExpiredPhoto, messageExpiredVideo, messageGame, messageGameScore, '
    'messageInvoice, messageLocation, messagePassportDataReceived, '
    'messagePassportDataSent, messagePaymentSuccessful, '
    'messagePaymentSuccessfulBot, messagePhoto, messagePinMessage, '
    'messagePoll, messageProximityAlertTriggered, messageScreenshotTaken, '
    'messageSticker, messageSupergroupChatCreate, messageText, '
    'messageUnsupported, messageVenue, messageVideo, messageVideoNote, '
    'messageVoiceNote, messageWebsiteConnected'
)


text_entity_types = (
    'textEntityTypeBankCardNumber, textEntityTypeBold, textEntityTypeBotCommand, '
    'textEntityTypeCashtag, textEntityTypeCode, textEntityTypeEmailAddress, '
    'textEntityTypeHashtag, textEntityTypeItalic, textEntityTypeMention, '
    'textEntityTypeMentionName, textEntityTypePhoneNumber, textEntityTypePre, '
    'textEntityTypePreCode, textEntityTypeStrikethrough, textEntityTypeTextUrl, '
    'textEntityTypeUnderline, textEntityTypeUrl'
)


def _generate_fields(field_list):
    """Выводит в stdout поля для MessageContent из message_content
    в виде
        MESSAGE_ANIMATION = 'messageAnimation'
    Чтоб вручную не делать :)
    """

    regex = re.compile(r'([a-z])([A-Z])')
    for field in field_list.split(', '):
        field_upper = regex.sub(r'\1_\2', field).upper()
