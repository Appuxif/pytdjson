...
class MessageVideo(MessageContentBase):
    ...
    alternative_videos: List[AlternativeVideoFile] = field(
        default=None,
        metadata={
            'getter': lambda items: [AlternativeVideoFile(item) for item in items]
        },
    )
    storyboards: List[VideoStoryboardFile] = field(
        default=None,
        metadata={
            'getter': lambda items: [VideoStoryboardFile(item) for item in items]
        },
    )
    # cover:photo
...
class MessageGroupCall(MessageContentBase):
    ...
    is_video: bool = None
    duration: int = None
    other_participant_ids: List[MessageSender] = field(
        default=None,
        metadata={'getter': lambda items: [MessageSender(item) for item in items]},
    )
